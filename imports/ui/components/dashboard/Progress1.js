...
class Progress1 extends Component {
  render() {
          ...
          switch (course.type) {
              ...
              switch (course.dept) {
                ...
                default:
                  if (
                    ...
                    course.dept === "COMM" ||
                    course.dept === "SCIE" ||
                    (course.dept === "BUSI" && course.num >= 300)
                  )
            ...
            case "exemptions":
              switch (course.dept) {
                ...
                case "MATH":
                  ...
                  return;
                case "STAT":
                  if (course.num === 200 || course.num === 203) {
                    user.requirements.core.STAT = "complete";
                    user.requirements.core.counter++;
                  }
                  return;
            ...
            case "electives":
              switch (course.dept == "CPSC") {
                case course.num < 300:
                  console.log(
                    "Not a valid elective " + course.dept + " " + course.num
                  );
                  console.log(
                    "Usage: must be CPSC 300 or higher, and not a core requirement"
                  );
                  return;
                case course.num < 400:
                  if (!course.consumed) {
            ...
            case "replacement":
              ...
              if (user.requirements.replacements.includes(courseLevel)) {
                user.requirements.replacements.splice(
                  user.requirements.replacements.indexOf(courseLevel),
                  1
                );
                console.log(
                  "used " +
                ...
                user.requirements.replacements.splice(
                  user.requirements.replacements.indexOf(
                    Math.max(...user.requirements.replacements)
                  ),
                  1
                );
                console.log(
                ...
                // course num must be too low to quailify
                console.log(
                  course.dept +
                    " " +
                    course.num +
                    " was not a valid exemption replacement"
                );
              }
              return;
          } // end switch
    ...
    // stop infinite loop
    if (this.componentDidUpdate) Users.update({ _id: user._id }, user);
    ...
    if (user !== undefined) {
      ...
      console.log(user);
      coreComplete = user.requirements.core.counter;
    }
    ...
    const bridgingComplete = user
      ? user.requirements.bridging.CPSC + user.requirements.bridging.OTHER
      : "";
    ...
    const electiveComplete = user
      ? user.requirements.elective[0] + user.requirements.elective[1]
      : "";
    ...
    function makeView(course) {
      return (
        <p className="complete" key={shortid.generate()}>
    ...
    return user ? (
      <div>
        <div className="ui bottom attached segment active tab">
          ...
          <p className={user.requirements.core.COMM}>
